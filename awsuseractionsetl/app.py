...
class AwsUseractionReports:

    def __init__(self):
        self.file_count = self.file_max_count = 10000000
        self.file_count = 0
        self.out_folder = "/tmp/useraction"
        self.temp_out_folder = self.out_folder + "/reports_temp/"
        self.useraction_csv_folder = self.out_folder + "/reports_combined/"
        self.data_folder = self.out_folder + "/data/"
        self.dest_s3_bucket = "duplo-useraction-reports"
        self.start = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        self.s3_client = boto3.client('s3')
        self.useraction_report_files = []
        self.empty_buckets = []

    def etl_on_customer_useraction_s3_buckets(self, upload_s3_folder):
        self._log(self.file_count, "temp_out_folder ", self.temp_out_folder)
        self._log(self.file_count, "useraction_csv_folder", self.temp_out_folder)
        self._log(self.file_count, "data_folder", self.data_folder)
        self.recreate_local_folder(self.temp_out_folder)
        self.recreate_local_folder(self.useraction_csv_folder)
        self.recreate_local_folder(self.data_folder)

        #s3_file_prefix = "data/useraction-data-12months/last-12-months/"
        s3_file_prefix = "data/useraction/all/"
            ...
            for page in paginator.paginate(Bucket=bucket):
                ...
                for obj in page.get('Contents', []):
                    ...
                    if s3_file_prefix in file_name and "json" in file_name:
                        self.file_count += 1
                        s3_useraction_file = S3UseractionFile(self.file_count, bucket, file_name, file_size, file_date, self.data_folder)
                        self._process_s3_useraction_file(s3_useraction_file)
                        self.useraction_report_files.append(s3_useraction_file)

        # save empty buckets
        self._save_empty_buckets_csv()
        # save processed files
        self._save_useraction_report_csv()
        # Upload csvs
        self.upload_csv_files(upload_s3_folder)

    def _process_s3_useraction_file(self, s3_useraction_file):
        self.file_count += 1
        self.file_count = s3_useraction_file.file_index
        dest_s3_object_key = upload_s3_folder + "/" + s3_useraction_file.file_name
        self._log(self.file_count,
                  "_process_s3_useraction_file " + s3_useraction_file.bucket_name
                  + " src: " + s3_useraction_file.s3_object_key
                  + " dest: " + dest_s3_object_key)
        copy_source = {
            'Bucket': s3_useraction_file.bucket_name,
            'Key': s3_useraction_file.s3_object_key
        }
        s3.meta.client.copy(copy_source, self.dest_s3_bucket, dest_s3_object_key)
        s3.Object(s3_useraction_file.bucket_name, s3_useraction_file.s3_object_key).delete()

    def upload_csv_files(self, upload_s3_folder):
        for csv_file in os.listdir(self.useraction_csv_folder):
            if csv_file.endswith(".csv"):
                file_path = self.useraction_csv_folder + csv_file
    ...
    def _save_empty_buckets_csv(self):
        empty_buckets_file = self.useraction_csv_folder + "empty_buckets.csv"
    ...
    def _save_useraction_report_csv(self):
        useraction_report_file = self.useraction_csv_folder + "useraction_report_files.csv"
        useraction_report_json = json.dumps([{
        ...
        } for s3_file in self.useraction_report_files])
        sum_df = pd.read_json(useraction_report_json)
        sum_df.to_csv(useraction_report_file, index=False, header=True)
...
def handler(event, context):
    aws_useraction_reports = AwsUseractionReports()
    aws_useraction_reports.etl_on_customer_useraction_s3_buckets("aws")
    timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    return f"total reports: {str(aws_useraction_reports.file_count)}, start: {aws_useraction_reports.start}, end: {timestamp}, v:{sys.version} !"
